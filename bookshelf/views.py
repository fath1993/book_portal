...
from book_portal.settings import SITE_URL_LOGIN, SITE_URL
from bookshelf.models import Book, FeaturedBook, BookAssign, BookVisitedNumber
...
def home_view(request):
    context = {}
    books = Book.objects.filter(is_published_on_site=True).order_by('id')[:12]
    context['books'] = books
...
def book_view(request, book_id, book_name):
    ...
    book_assign_history = BookAssign.objects.filter(book=book)
...
class BookVisitThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            logger('object is busy. err: ' + str(e), 'd')
            return False
